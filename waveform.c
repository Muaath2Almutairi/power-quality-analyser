...
// The functions use a field_offset parameter (from offsetof) to know which
// phase to look at.
...
static double get_voltage(const WaveformSample *p, int field_offset) {
  // convert to char pointer so we can add the byte offset
  const char *base = (const char *)p;

  // create an empty double to hold our answer
  double value;

  // Use memcpy to safely copy the 8 bytes of data into our empty variable
...
double compute_std_dev(const WaveformSample *samples, int n, int field_offset,
                       double mean) {
  if (n <= 0)
    return 0.0;
...
PhaseResult analyse_phase(const WaveformSample *samples, int n, char label,
                          int field_offset) {
  PhaseResult r;
  r.label = label;
  ...
  // Flags for merit extension
  r.status_flags = 0;
  if (r.clipped_count > 0)
    r.status_flags = r.status_flags + FLAG_CLIPPING;
  if (!r.compliant)
    r.status_flags = r.status_flags + FLAG_OUT_OF_TOL;
